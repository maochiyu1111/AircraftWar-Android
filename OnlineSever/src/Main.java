...
public class Main {
    ...
    private static void acceptConnections1(ServerSocket serverSocket) throws IOException, ClassNotFoundException {
        while (true) {
            ...
            // 处理Socket连接
            new Thread(new LogSever(socket)).start();
        }
    }
    ...
    private static void acceptConnections2(ServerSocket serverSocket) throws IOException, ClassNotFoundException {
        while (true) {
            ...
            // 处理Socket连接
//            new Thread(new OnlineSever(socket1,socket2, LogSever.map.get(1), LogSever.map.get(2))).start();
//            new Thread(new OnlineSever(socket2,socket1, LogSever.map.get(2), LogSever.map.get(1))).start();
        }
    }
