    ...
    public List<BaseBullet> shootBullet(AbstractAircraft aircraft, int direction, int power, int shootNum) {
        ...
        if (direction == -1){
            for(int i=0; i<shootNum; i++){
                // 子弹发射位置相对飞机位置向前偏移
                // 子弹具有横向速度
                bullet = new HeroBullet(x + (i*2 - shootNum + 1)*40, y, aircraft.getSpeedX()+ (i -shootNum /2 )*5 , -30, power);
                res.add(bullet);
            }
        }
        else {
            for(int i=0; i<shootNum; i++){
                // 子弹发射位置相对飞机位置向前偏移
                // 子弹具有横向速度
                bullet = new EnemyBullet(x + (i*2 - shootNum + 1)*40, y, aircraft.getSpeedX()+ (i -shootNum /2 )*5 , speedY, power);
                res.add(bullet);
            }
        }
        return res;
